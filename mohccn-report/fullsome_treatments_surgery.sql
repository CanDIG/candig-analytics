...
WHERE mohpackets_donor.program_id_id IS NOT NULL
  ...
  AND anatomical_site_irradiated IS NOT NULL) TO '/tmp/fullsome_treatments_surgery_completeness.csv' with (FORMAT CSV, HEADER);
COPY (SELECT program_id_id, submitter_donor_id, COUNT(*)
FROM program_id_id, mohpackets_surgery
